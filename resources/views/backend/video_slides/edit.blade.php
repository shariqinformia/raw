...
@section('title', 'Video Slide')

@section('breadcump')
    <div class="col-sm-6">
        <h1 class="m-0">{{ __('Video Slide') }}</h1>
    </div>
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('backend.dashboard.index') }}">{{ __('Home') }}</a></li>
            <li class="breadcrumb-item">{{ __('Video Slide') }}</li>
...
@section('main')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('backend.video_slides.update', $video_slide) }}" enctype="multipart/form-data" method="POST"
                  id="{{($idFormEdit == true) ? 'editFormHandler': ''}}">
                @csrf
                @method('PUT')
                @include('backend.video_slides._form')
                <div class="card mb-4">
                    <div class="card-body">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title" id="deleteUserLabel">{{ __('Delete Video Slide') }}</h5>
                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                    <span aria-hidden="true">&times;</span>
                                </button>
                            </div>
                            <form action="{{ route('backend.video_slides.destroy', $video_slide) }}" method="POST">
                                @csrf
                                @method('DELETE')
